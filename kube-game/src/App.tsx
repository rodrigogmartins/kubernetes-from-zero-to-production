...
export default function App() {
  const [state, setState] = useState<ActualState>(createInitialState())
    ...
    const interval = setInterval(() => {
      setState((prev) => {
        ...
        const nextTask = phase1Tasks.find(
          (task) => !completedTasks.has(task.id) && task.isCompleted(next)
        )
        if (nextTask) {
          ...
          setTimeout(() => setShowTaskNotification(false), 3000)
        }
        return next
      })
    }, 1000)
  ...
  function killNode(nodeId: string) {
    setState((prev) => ({
      ...
      nodes: prev.nodes.map((n) =>
        n.id === nodeId ? { ...n, status: 'Down', pods: [] } : n
      ),
    }))
  }
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              ...
              <p className="text-xs md:text-sm text-muted-foreground hidden md:block">
                Learn Kubernetes fundamentals through gameplay
              </p>
            </div>
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-0 overflow-hidden">
          ...
          <div className="w-full lg:w-80 flex flex-col border-t lg:border-t-0 lg:border-l border-border bg-background overflow-hidden">
            ...
            <div
              ...
              } lg:flex flex-col overflow-hidden flex-1`}
            >
              {/* Active Task */}
